...
class UserUpdate(BaseModel):
    ...
    show_on_leaderboard: Optional[bool] = Field(
        None, description="Whether to show user on leaderboard"
    )
    display_name: Optional[str] = Field(
        None, max_length=255, description="Display name for leaderboard"
    )
...
class FocusSessionBase(BaseModel):
    category: str = Field(
        ...
        min_length=1,
        max_length=50,
        description="Focus category (e.g., Work, Study)",
    )
...
class FocusGoalBase(BaseModel):
    category: str = Field(
        ..., min_length=1, max_length=50, description="Focus category"
    )
    goal_type: GoalType = Field(
        ..., description="Goal type: TIME_BASED, DAILY_CHECKBOX, or WEEKLY_CHECKBOX"
    )
    goal_time_per_week_seconds: Optional[int] = Field(
        None,
        ge=0,
        le=604800,
        description="Weekly goal in seconds (max 168 hours, only for TIME_BASED)",
    )
    description: Optional[str] = Field(
        None, max_length=255, description="Description for checkbox goals"
    )
...
class FocusGoalUpdate(BaseModel):
    goal_time_per_week_seconds: Optional[int] = Field(
        None, ge=0, le=604800, description="New weekly goal in seconds (max 168 hours)"
    )
    description: Optional[str] = Field(
        None, max_length=255, description="New description"
    )
...
class CategoryRename(BaseModel):
    ...
    new_category: str = Field(
        ..., min_length=1, max_length=50, description="New category name"
    )
...
class CategoryRenameResponse(BaseModel):
    ...
    requires_merge: bool = Field(
        ..., description="True if target category exists and merge is needed"
    )
    target_exists: bool = Field(
        ..., description="True if target category already exists"
    )
